...
class MultiTaskNet(nn.Module):
    ...
    def init_mlp_layers(self, layer_sizes):
        ...
        for fan_in, fan_out in zip(layer_sizes, layer_sizes[1:]):
            # build each layer, linear + relu so that values are sensitive to the non-linearity
            mlp_layers.append(nn.Linear(fan_in, fan_out))
    ...
    def forward_with_embedding_sharing(self, user_ids, item_ids):
        ...
        # Regression head
        score = torch.cat((user_embed, item_embed, user_embed * item_embed), 1)
        for layer in self.mlp_layers:
            score = layer(score)
        score = score.squeeze()

        # Matrix Factorization Head  
        predictions = ((user_embed * item_embed).sum(1, keepdim=True) + self.B(item_ids)).squeeze(1)
    ...
    def forward_without_embedding_sharing(self, user_ids, item_ids):
        ...
        predictions = score = None
        ### START CODE HERE ###
        user_embed_reg, user_embed_fact = self.U_reg(user_ids), self.U_fact(user_ids)
        item_embed_reg, item_embed_fact = self.Q_reg(item_ids), self.Q_fact(item_ids)

        # Regression head
        score = torch.cat((user_embed_reg, item_embed_reg, user_embed_reg * item_embed_reg), 1)
        for layer in self.mlp_layers:
            score = layer(score)

        # Matrix Factorization Head 
        predictions = ((user_embed_fact * item_embed_fact).sum(1, keepdim=True) + self.B(item_ids)).squeeze(1)
